...
!function($, window, document, _undefined)
{
		...
		createOverlay: function($trigger, templateHtml, extraOptions)
		{
			...
			$trigger.bind(
			{
				...
				onLoad: function(e)
				{
					...
					var $focus = $overlay.find('form').find('input[autofocus], textarea[autofocus], select[autofocus]').first();
					if ($focus.length)
					{
						$focus.focus();
					}
	...
	XenForo.OverlayTrigger.prototype =
	{
		...
		show: function(e)
		{
			...
			if (this.$trigger.is('.control.edit')
				&& $.browser.mozilla && parseInt($.browser.version, 10) >= 23)
			{
				return true;
			}
	...
	XenForo.AutoValidator.prototype =
	{
		...
		redirect: function(redirectTarget)
		{
			if (XenForo.isPositive(this.$form.data('redirect')) || !parseInt(XenForo._enableOverlays))
			{
				...
				{
					XenForo.redirect($AutoValidationRedirect.redirectTarget);
					return true;
				}
			}
	...
	XenForo.register('input, textarea', function(i)
	{
		...
		if ($this.is('textarea.Elastic'))
		{
			...
			else if (!$this.attr('rows') || $this.attr('rows') <= 2)
			{
				$this.attr('rows', 6);
			}
		}
