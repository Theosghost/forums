...
!function($, window, document, _undefined)
{
	...
	XenForo.ColorPicker._formInit = {};
	XenForo.ColorPicker.prototype =
	{
		__construct: function($input)
		{
			var $form = $input.closest('form'), formId = $form.attr('id');
			if (!formId)
			{
				formId = XenForo.uniqueId();
				$form.attr('id', formId);
			}

			if (!XenForo.ColorPicker._formInit[formId] && !$form.data('XenForo.ColorPickerForm'))
			{
				...
				XenForo.ColorPicker._formInit[formId] = true;
			}
			this.$form = $form;
