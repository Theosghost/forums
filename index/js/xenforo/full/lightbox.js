...
!function($, window, document, _undefined)
{
	...
	{
		var $lightBox = $('#LightBox').attr('unselectable', true),
		...
		this.setThumbStrip = function($container)
		{
			...
			$thumbStrip.find('li').not($thumbTemplate).xfRemove('hide', null, 0);
		...
		this.setImage = function($thumb, animationSpeed)
		{
			...
			$pic.one('load', function() { setTimeout(loadFunc, 0); }).error(function() {
				$thumbStrip.find('li:not(#LbThumbTemplate) a').each(function(i, thumb)
				{
					if ($(thumb).data('src') == imageSource)
					{
						...
						{
							console.log($($thumbStrip.find('li:not(#LbThumbTemplate) a').get(Math.max(0, i - 1))).data('el'));
							self.setImage(
